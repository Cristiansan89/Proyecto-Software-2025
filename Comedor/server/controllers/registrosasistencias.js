...
import {
  ...
  validatePartialRegistroAsistencia,
} from "../schemas/registrosasistencias.js";
...
export class RegistroAsistenciaController {
  ...
  // Obtiene todos los RegistrosAsistencias con filtros opcionales
  getAll = async (req, res) => {
    try {
      ...
      if (idServicio) filtros.idServicio = idServicio;
      if (idGrado) filtros.idGrado = idGrado;
  ...
  getById = async (req, res) => {
    try {
      const { id } = req.params;
      const registroAsistencia = await this.registroAsistenciaModel.getById({
        id,
      });
  ...
  create = async (req, res) => {
    try {
      ...
      const newRegistroAsistencia = await this.registroAsistenciaModel.create({
        input: result.data,
      });
  ...
  update = async (req, res) => {
    try {
      ...
      const updatedRegistroAsistencia =
        await this.registroAsistenciaModel.update({ id, input: result.data });
  ...
  getByFecha = async (req, res) => {
    try {
      ...
      const asistencias =
        await this.registroAsistenciaModel.getAsistenciasByFecha({ fecha });
  ...
  getByPersona = async (req, res) => {
    try {
      ...
      const asistencias = await this.registroAsistenciaModel.getByPersona({
        id_persona,
      });
      res.json(asistencias);
    } catch (error) {
  ...
  getByServicioFecha = async (req, res) => {
    try {
      ...
      const asistencias = await this.registroAsistenciaModel.getByServicioFecha(
        { id_servicio, fecha }
      );
      res.json(asistencias);
    } catch (error) {
      console.error(
        "Error al obtener asistencias por servicio y fecha:",
        error
      );
      res.status(500).json({ message: "Error interno del servidor" });
    }
  ...
  marcarAsistencia = async (req, res) => {
    try {
      ...
      const asistencia = await this.registroAsistenciaModel.marcarAsistencia({
        id_persona,
        id_servicio,
        fecha,
      });
  ...
  getEstadisticas = async (req, res) => {
    try {
      ...
      // Si se proporciona solo fecha, usar como fecha específica
      const parametros = {};
      if (fecha) {
      ...
      const estadisticas = await this.registroAsistenciaModel.getEstadisticas(
        parametros
      );
  ...
  // Verificar si todas las asistencias están registradas
  verificarCompletas = async (req, res) => {
    try {
      ...
      const resultado =
        await this.registroAsistenciaModel.verificarAsistenciasCompletas({
          fecha,
          idServicio,
        });
