...
export class PersonaController {
  ...
  delete = async (req, res) => {
    try {
      ...
      // Verificar si la persona tiene usuarios activos asociados
      const hasActiveUsers = await this.personaModel.hasActiveUsers({ id });
      if (hasActiveUsers) {
        return res.status(409).json({
          message:
            "No se puede eliminar la persona porque está vinculada a registros activos",
        });
      }
  ...
  update = async (req, res) => {
    try {
      ...
      const updatedPersona = await this.personaModel.update({
        id,
        input: result.data,
      });
  ...
  getByDocumento = async (req, res) => {
    try {
      ...
      const persona = await this.personaModel.getByDocumento({
        numeroDocumento,
      });
      if (persona) return res.json(persona);
      res.status(404).json({ message: "Persona no encontrada" });
