import { generateTemporalPassword, sendPasswordResetEmail } from '../services/emailService.js';
...
export const forgotPassword = async (req, res, usuarioModel) => {
    try {
        ...
        if (!user.mail) {
            return res.status(400).json({
                success: false,
                message: 'El usuario no tiene un email asociado. Contacte al administrador.'
            });
        }
