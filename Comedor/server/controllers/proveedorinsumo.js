...
export class ProveedorInsumoController {
  ...
  getById = async (req, res) => {
    try {
      ...
      res
        .status(404)
        .json({ message: "Relación proveedor-insumo no encontrada" });
    } catch (error) {
  ...
  create = async (req, res) => {
    try {
      ...
      const newProveedorInsumo = await this.proveedorInsumoModel.create({
        input: result.data,
      });
  ...
  delete = async (req, res) => {
    try {
      ...
      return res.json({
        message: "Relación proveedor-insumo eliminada correctamente",
      });
    } catch (error) {
  ...
  getInsumosByProveedor = async (req, res) => {
    try {
      ...
      const insumos = await this.proveedorInsumoModel.getByProveedor({
        id_proveedor,
      });
      res.json(insumos);
    } catch (error) {
  ...
  getProveedoresByInsumo = async (req, res) => {
    try {
      ...
      const proveedores = await this.proveedorInsumoModel.getByInsumo({
        id_insumo,
      });
  ...
  getMejorProveedorByInsumo = async (req, res) => {
    try {
      const { id_insumo } = req.params;
      const proveedores = await this.proveedorInsumoModel.getBestProviders({
        id_insumo,
      });
