...
export class TurnoController {
  ...
  delete = async (req, res) => {
    try {
      ...
      // Verificar si el turno tiene relaciones activas
      const hasActiveRelations = await this.turnoModel.hasActiveRelations({
        id,
      });
      if (hasActiveRelations) {
        return res.status(409).json({
          message:
            "No se puede eliminar el turno porque está vinculado a registros activos",
        });
      }
  ...
  update = async (req, res) => {
    try {
      ...
      const { id } = req.params;
      const updatedTurno = await this.turnoModel.update({
        id,
        input: result.data,
      });
  ...
  changeStatus = async (req, res) => {
    try {
      ...
      res.json({
        message: `Turno ${estado.toLowerCase()} correctamente`,
        turno: updatedTurno,
      });
    } catch (error) {
