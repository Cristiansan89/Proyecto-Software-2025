...
export class GradoController {
  ...
  create = async (req, res) => {
    try {
      ...
      if (!result.success) {
        console.log(
          "GradoController: Errores de validación:",
          result.error.issues
        );
  ...
  update = async (req, res) => {
    try {
      ...
      const { id } = req.params;
      const updatedGrado = await this.gradoModel.update({
        id,
        input: result.data,
      });
  ...
  getActivos = async (req, res) => {
    try {
      ...
      const gradosActivos = allGrados.filter(
        (grado) => grado.estado === "Activo"
      );
  ...
  cambiarEstado = async (req, res) => {
    try {
      ...
      const gradoActualizado = await this.gradoModel.update({
        id,
        input: { estado },
      });
      if (!gradoActualizado) {
  ...
  changeStatus = async (req, res) => {
    try {
      ...
      res.json({
        message: `Grado ${estado.toLowerCase()} correctamente`,
        grado: updatedGrado,
      });
    } catch (error) {
      console.error("Error al cambiar estado del grado:", error);
