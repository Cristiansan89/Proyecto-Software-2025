...
export class MovimientoInventarioModel {
    ...
    static async create({ input }) {
        ...
        try {
            const [result] = await connection.query(
                ...
                ) VALUES (?, UUID_TO_BIN(?), ${id_consumo ? 'UUID_TO_BIN(?)' : 'NULL'}, ?, ?, ?, ?);`,
                id_consumo
                    ? [id_insumo, id_usuario, id_consumo, id_tipoMerma, tipoMovimiento, cantidadMovimiento, comentarioMovimiento]
                    : [id_insumo, id_usuario, id_tipoMerma, tipoMovimiento, cantidadMovimiento, comentarioMovimiento]
            )
    ...
    static async registrarMovimientoConActualizacion({ input }) {
        ...
        try {
            ...
            const signo = tipoMovimiento.toLowerCase().includes('entrada') || tipoMovimiento.toLowerCase().includes('compra') ? '+' : '-'
