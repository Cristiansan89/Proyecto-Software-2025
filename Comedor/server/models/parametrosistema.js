...
export class ParametroSistemaModel {
  ...
  static async create({ input }) {
    const { nombreParametro, valor, tipoParametro, estado = "Activo" } = input;
  ...
  static async update({ id, input }) {
    const { nombreParametro, valor, tipoParametro, estado } = input;
  ...
  static async updateByNombre({ nombreParametro, valor }) {
    try {
      const [result] = await connection.query(
        `UPDATE Parametros 
                 SET valor = ?, fechaModificacion = CURRENT_TIMESTAMP
                 WHERE nombreParametro = ?;`,
        [valor, nombreParametro]
      );
      return result.affectedRows > 0;
    } catch (error) {
      console.error("Error al actualizar parámetro por nombre:", error);
      throw new Error("Error al actualizar parámetro por nombre");
    }
  }
}
