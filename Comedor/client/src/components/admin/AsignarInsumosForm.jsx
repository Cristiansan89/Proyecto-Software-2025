...
const AsignarInsumosForm = ({ proveedor, onSave, onCancel }) => {
    ...
    const loadCalificaciones = async () => {
        try {
            const calificacionesFromServer = await proveedorService.getCalificaciones();
    ...
    const loadInsumos = async () => {
        try {
            ...
            const insumosActivos = insumos.filter(insumo => insumo.estado === 'Activo');
    ...
    const loadInsumosAsignados = async () => {
        ...
        try {
            const insumosAsignados = await proveedorService.getInsumosAsignados(proveedor.idProveedor);
            setAssignedInsumos(insumosAsignados);
        } catch (error) {
            console.error('Error al cargar insumos asignados:', error);
                        ...
                        <div className="info-card">
                            ...
                            <div className="info-grid">
                                <div>
                                    ...
                                    <span className="info-value">{proveedor?.razonSocial || 'No especificado'}</span>
                                </div>
                                <div>
                                    <i className="fas fa-id-card me-2"></i>
                                    <span className="info-label">CUIT: </span>
                                    <span className="info-value">{proveedor?.CUIT || 'No especificado'}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="insumos-list available-list">
                            {filteredInsumos.length === 0 ? (
                                <div className="no-data">
                                    <p className="text-muted">No hay insumos disponibles para asignar</p>
                                </div>
                            ) : (
                                filteredInsumos.map(insumo => (
                                    <div key={insumo.idInsumo} className="insumo-item available-item">
                                        <div className="insumo-info">
                                            ...
                                            <div className="insumo-details">
                                                <span className="categoria-badge">{insumo.categoria}</span>
                                                <span className="unidad-medida">{insumo.unidadMedida}</span>
                                            </div>
                                        </div>
                                        <button
                                            type="button"
                    ...
                    <div className="assigned-insumos">
                        ...
                        <div className="insumos-list assigned-list">
                            {assignedInsumos.length === 0 ? (
                                <div className="no-data">
                                    <p className="text-muted">No hay insumos asignados a este proveedor</p>
                                </div>
                            ) : (
                                assignedInsumos.map(insumo => (
                                    <div key={insumo.idInsumo} className="insumo-item assigned-item">
                                        <div className="insumo-info">
                                            <div className="insumo-name">
                                                <i className="fas fa-box me-2"></i>
                                                <strong>{insumo.nombreInsumo}</strong>
