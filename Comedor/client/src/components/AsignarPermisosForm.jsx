...
const AsignarPermisosForm = ({ onClose, rolSeleccionado = null }) => {
    ...
    const cargarPermisosDelRol = useCallback(async () => {
        ...
        try {
            const permisosDelRol = await rolPermisoService.getPermisosByRol(selectedRol);
            setPermisosActuales(permisosDelRol);
            setSelectedPermisos(permisosDelRol.map(p => p.idPermiso || p.id_permiso));
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await rolPermisoService.limpiarPermisosRol(selectedRol);

            // Asignar nuevos permisos si hay alguno seleccionado
            if (selectedPermisos.length > 0) {
                await rolPermisoService.asignarPermisos(selectedRol, selectedPermisos);
            }

            setSuccess(`Permisos actualizados exitosamente. ${selectedPermisos.length} permisos asignados.`);
                ...
                <div className="form-grid">
                        ...
                        >
                            ...
                            {roles.map(rol => (
                                <option key={rol.idRol} value={rol.idRol}>
                                    {rol.nombreRol} - {rol.descripcionRol}
                                </option>
                            ))}
                        </select>
                    ...
                    {selectedRol && (
                        <div className="form-group">
                            ...
                            <small className="form-help">
                                Filtre los permisos por módulo del sistema para una mejor organización
                            </small>
                        </div>
                    )}
                </div>
                ...
                {selectedRol && (
                    <div className="rol-info">
                        ...
                        <div className="text-muted">
                            ...
                            <strong> Permisos seleccionados:</strong> {selectedPermisos.length} |
                            <strong> Total disponibles:</strong> {permisos.length}
                        </div>
                    </div>
                )}
                                ...
                                <div key={modulo} className="modulo-section">
                                    <div className="modulo-header">
                                        <label className="checkbox-label">
                                            <input
                                                ...
                                                onChange={(e) => handleSelectAllModule(modulo, e.target.checked)}
                                            />
                                    ...
                                    <div className="permisos-grid">
                                        {permisosDelModulo.map(permiso => {
                                            ...
                                            return (
                                                <label key={idPermiso} className="checkbox-label permiso-item">
                                                    <input
                                                        type="checkbox"
                                                        checked={isPermisoSelected(idPermiso)}
                                                        onChange={(e) => handlePermisoChange(idPermiso, e.target.checked)}
                                                    />
