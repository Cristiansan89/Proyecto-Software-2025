...
export function ForgotPassword({ onBack }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage(
          "Se ha enviado una nueva contraseña a su correo electrónico. Por favor revise su bandeja de entrada.",
        );
        setIsError(false);
  ...
  return (
    <div className={LoginStyle.forgotPassword}>
      <div className={LoginStyle.containerPassword}>
        <div className={LoginStyle.headerPassword}>
          <h2>Recuperar Contraseña</h2>
          <p>
            Ingrese su nombre de usuario y le enviaremos una nueva contraseña a
            su correo electrónico
          </p>
        </div>

        <form onSubmit={handleSubmit} className={LoginStyle.formPassword}>
          ...
          {message && (
            <div
              className={`${LoginStle.message} ${isError ? LoginStyle.messageError : LoginStyle.messageSuccess}`}
            >
              {message}
            </div>
          )}
