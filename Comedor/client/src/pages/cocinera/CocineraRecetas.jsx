...
const CocineraRecetas = () => {
    ...
    const loadRecetaIngredientes = async (recetaId) => {
        try {
            ...
            setFilteredRecetas(prev => prev.map(receta =>
                receta.id_receta === recetaId
                    ? { ...receta, ingredientes }
                    : receta
            ));
    ...
    useEffect(() => {
        ...
        if (searchQuery.trim()) {
            filtered = filtered.filter(receta =>
                receta.nombreReceta.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const handleDelete = async (receta) => {
        if (!confirm(`¿Está seguro de que desea eliminar la receta "${receta.nombreReceta}"?`)) {
            return;
        }
                                ...
                                <div key={receta.id_receta} className="col-lg-3 col-md-6 mb-4">
                                    <div className="card h-100">
                                        <div className="card-body p-0">
                                            <div className="accordion" id={`accordion-${receta.id_receta}`}>
                                                {/* Header de la receta */}
                                                <div className="accordion-item border-0">
                                                    <h5 className="accordion-header bg-primary text-white p-3 mb-0">
                                                        <div className="d-flex justify-content-between align-items-center">
                                                            ...
                                                            <span className={`badge ${receta.estado === 'Activo' ? 'bg-success' : 'bg-secondary'}`}>
                                                                {receta.estado}
                                                            </span>
                                                        </div>
                                                ...
                                                <div className="accordion-item">
                                                    <h2 className="accordion-header" id={`heading-instrucciones-${receta.id_receta}`}>
                                                        <button
                                                ...
                                                {/* Ingredientes */}
                                                <div className="accordion-item">
                                                    <h2 className="accordion-header" id={`heading-ingredientes-${receta.id_receta}`}>
                                                        <button
                                                            className="accordion-button collapsed"
                                                            type="button"
                                                            data-bs-toggle="collapse"
                                                            ...
                                                            onClick={() => loadRecetaIngredientes(receta.id_receta)}
                                                        >
                                                    ...
                                                    >
                                                        <div className="accordion-body">
                                                            {receta.ingredientes && receta.ingredientes.length > 0 ? (
                                                                <div className="small">
                                                                    {receta.ingredientes.map((ingrediente, ingIndex) => (
                                                                        <div key={ingIndex} className="mb-2 d-flex justify-content-between">
                                                                            <span>
                                                                                <strong>{ingIndex + 1}. </strong>
                                                                                {ingrediente.nombreInsumo}
                                                                            </span>
                                                                            <span className="text-muted">
                                                                                {ingrediente.cantidadPorPorcion} {ingrediente.unidadPorPorcion}
                                                                            </span>
                                                                        </div>
                                                                    ))}
                                                                </div>
                        ...
                        {totalPages > 1 && (
                            <div className="d-flex justify-content-between align-items-center mt-3">
                                <div className="showing-info">
                                    <small className="text-muted">
                                        Mostrando {indexOfFirstItem + 1} a {Math.min(indexOfLastItem, filteredRecetas.length)} de {filteredRecetas.length} recetas
                                    </small>
                                </div>
                                <nav>
