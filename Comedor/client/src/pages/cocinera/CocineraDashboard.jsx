...
const CocineraDashboard = () => {
    ...
    const cargarDashboard = async () => {
        try {
            ...
            const docentes = docentesRes.data?.filter(persona => persona.nombreRol === 'Docente') || [];
            const grados = gradosRes.data?.filter(grado => grado.estado === 'Activo') || [];
            const servicios = serviciosRes.data?.filter(servicio => servicio.estado === 'Activo') || [];
    ...
    const generarServiciosHoy = (servicios) => {
        ...
        return horariosDelDia.map(horario => {
            ...
            return {
                ...
                hora: horario.hora,
                icono: horario.icono,
                estado: ahora > horaServicio ? 'completado' : ahora.getTime() < horaServicio.getTime() - 30 * 60 * 1000 ? 'pendiente' : 'en_curso',
                descripcion: servicio?.descripcion || `Servicio de ${horario.nombre.toLowerCase()}`,
                        ...
                        <div className="card-body">
                            <div className="servicios-timeline">
                                {serviciosHoy.map((servicio) => (
                                    <div key={servicio.id} className={`timeline-item estado-${servicio.estado}`}>
                                        ...
                                        <div className="timeline-content">
                                            <div className="d-flex justify-content-between align-items-start">
                                                ...
                                                <span className={`badge bg-${getEstadoColor(servicio.estado)}`}>
                            ...
                            {proximosServicios.map((servicio) => (
                                <div key={servicio.id} className="proximo-servicio">
                                    <div className="d-flex align-items-center">
                                        <span className="servicio-icono me-3">{servicio.icono}</span>
                                        <div>
